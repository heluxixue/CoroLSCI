# -*- coding: utf-8 -*-
"""fig11_cvr.py — 新增论文图 fig11_cvr.pdf（CVR 生理参照，三联 A/B/C）

风格与 fig10_longitudinal / paper_fig_polish.py 一致:
Times New Roman 衬线 + stix 数学体; INK=#0b0b0b(黑色=rig CVR) MUTED=#555(灰=rig CF)
ACC=#2a78d6(蓝=LSCI 光学链)。数据源: 灌注日志 CSV + 表III/表V 会话值。
不修改任何已有图文件。
"""
import csv
import sys
...
# label each CVR session marker at its own side (right-axis coords);
# offsets chosen to clear the CF curve and the CVR segments
LBL = {"T0": (1.5, 8.66, "left"), "T1": (4.5, 8.44, "left"),
       "T2": (12.5, 8.10, "left"), "T3": (16.5, 7.90, "left"),
       "T4": (45.5, 7.82, "left"), "T5": (82.0, 7.72, "left"),
       "T6": (111.5, 8.62, "right")}
for mm, lab in [(0, "T0"), (3, "T1"), (11, "T2"), (15, "T3"), (44, "T4"), (79, "T5"), (113, "T6")]:
    ax.axvline(mm, color=INK, lw=0.5, alpha=0.30, zorder=1)
    lx, ly, halign = LBL[lab]
    ax2.text(lx, ly, lab, fontsize=8.5, ha=halign, va="center",
             color=INK, zorder=5,
...
for a_, txt_ in zip(axes, ["(A) Heart 1: session series",
                           "(B) Heart 1: rig flow trajectory",
                           "(C) Heart 2: 15-min replication"]):
    pos = a_.get_position()
    fig.text(pos.x0 + pos.width / 2, 0.022, txt_,
